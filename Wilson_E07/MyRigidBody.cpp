...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//Array of vector3's making up the cube, 8 points
	vector3 corners[8];
	...
	//Convert cube points to world space
	for (uint i = 0; i < 8; ++i)
	{
		corners[i] = vector3(m_m4ToWorld * vector4(corners[i], 1.0f));
	}

	//The max and the min start off at the first point
	m_v3MaxG = m_v3MinG = corners[0];

	//Find the real max and real min, replace with lower or higher values
	for (uint i = 1; i < 8; ++i)
	{
