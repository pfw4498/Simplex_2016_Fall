...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::Add:
		//You can't have more than 4 levels, because that's inefficient
		if (m_uOctantLevels < 4)
		{
			//Increase the level of octants, and safe delete the current root
			m_pEntityMngr->ClearDimensionSetAll();
			++m_uOctantLevels;
			SafeDelete(root);
			//Make a new root with the new amount of octant levels
			root = new MyOctant(m_uOctantLevels, 5);
		}
		break;
	case sf::Keyboard::Subtract:
		//You can't have 0 or less octant levels, because why would you do that?
		if (m_uOctantLevels > 0)
		{
			//Decrease the octant levels, safe delete the root
			m_pEntityMngr->ClearDimensionSetAll();
			--m_uOctantLevels;
			SafeDelete(root);
			//Create a new root, with the new level of octants
			root = new MyOctant(m_uOctantLevels, 5);
		}
		break;
	case sf::Keyboard::LShift:
