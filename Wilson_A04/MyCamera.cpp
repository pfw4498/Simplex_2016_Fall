...
void Simplex::MyCamera::MoveForward(float amount)
{
	//Move forward
	// A negative 'amount' value means move backward
	m_v3Position += m_v3Forward * amount;
	m_v3Target += m_v3Forward * amount;
}

void Simplex::MyCamera::MoveSideways(float amount)
{
	//Move to the right
	// A negative 'amount' value means move left
	m_v3Position += m_v3Right * amount;
	m_v3Target += m_v3Right * amount;
}

void Simplex::MyCamera::ChangeYaw(float amount)
{
	//Rotate Y
	m_v3Forward = glm::rotateY(m_v3Forward, amount);
	m_v3Target = glm::rotateY(m_v3Target, amount);
	m_v3Up = glm::rotateY(m_v3Up, amount);
	m_v3Right = glm::rotateY(m_v3Right, amount);
	m_v3Position = glm::rotateY(m_v3Position, amount);
}

void Simplex::MyCamera::ChangePitch(float amount)
{
	//Rotate X
	m_v3Forward = glm::rotateX(m_v3Forward, amount);
	m_v3Target = glm::rotateX(m_v3Target, amount);
	m_v3Up = glm::rotateX(m_v3Up, amount);
	m_v3Right = glm::rotateX(m_v3Right, amount);
	m_v3Position = glm::rotateX(m_v3Position, amount);
}
