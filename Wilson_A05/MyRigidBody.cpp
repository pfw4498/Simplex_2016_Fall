...
bool MyRigidBody::IsColliding(MyRigidBody* const a_pOther)
{
	...
	if (bColliding)
	{
		if(SAT(a_pOther) != 1)//eSATResults::SAT_NONE)
...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	*/
	...
	//Floats to store values for axes tests
	float ra;
	float rb;

	//The Rotation matrices are defined here
	matrix4 R = a_pOther->GetModelMatrix();// *m_m4ToWorld;
	matrix4 AbsR;

	//The distance from object B's center to object A's center
	vector3 t = a_pOther->GetCenterGlobal() - GetCenterGlobal();

	//Set up the absolute value matrix
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
	...
	//Here, we start the axis tests using cross products
	...
	//I want to return 1, so I don't get confused
	return 1;
}